# Accenture One Pager — Visual Template (Build Target)

This file is the **fixed visual specification** for the One Pager. The agent's behavior — how to read a CV/JD and tailor content — lives in the **project instructions**; this file is the reference the agent reads to reproduce the design **exactly**.

Every value below was extracted directly from the source file `Nuevo_OP.pptx`. **Do not vary the geometry, palette, fonts, bullets, or assets between outputs** — only the words change. Tailored content flows into this template via the **content schema (§9)**.

> **Confidence.** Geometry, colors, fonts, sizes, and bullet definitions are read straight from the file's XML — treat them as confirmed. Items I derived or substituted are flagged with ⚠️ and a recommendation; "verify against the reference" means render your output and eyeball it next to the original.
